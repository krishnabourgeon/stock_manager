...
//         "items": items,
//         "mode": summary?.mode ?? '',
//         // ✅ FIX: id is int? — send as int (0 if null)
//         "bill": summary?.id ?? 0,
//         "billdate": formattedDate,
//         "billtime": formattedTime,
...
// }
  ...
  Future<void> sharePdfToWhatsApp() async {
    ...
    final double discountInput =
        double.tryParse(provider.discountController.text) ?? 0.0;
        ...
        build: (context) {
          return pw.Column(
            ...
            children: [
              ...
              pw.Text("Details:\n${poojaString.join("\n")}"),
              pw.SizedBox(height: 10),
              // Sub Total (only show when discount is applied)
              if (discountInput > 0) ...[
                pw.Text("Sub Total: Rs ${subTotal.toStringAsFixed(2)}"),
                pw.SizedBox(height: 6),
                pw.Text(
                    "$discountLabel: - Rs ${discountValue.toStringAsFixed(2)}"),
                pw.SizedBox(height: 6),
              ],
              // GST breakdown (only show when applicable)
              if (cgst > 0) ...[
                pw.Text("CGST: Rs ${cgst.toStringAsFixed(2)}"),
                pw.SizedBox(height: 6),
              ],
              if (sgst > 0) ...[
                pw.Text("SGST: Rs ${sgst.toStringAsFixed(2)}"),
                pw.SizedBox(height: 6),
              ],
              if (gstTotal > 0) ...[
                pw.Text("GST Total: Rs ${gstTotal.toStringAsFixed(2)}"),
                pw.SizedBox(height: 6),
              ],
                    ...
                    TextButton(
                      onPressed: () {
                        if (context
                          ...
                          List<String> poojaString = [];
                          for (int i = 0; i < bp.person.length; i++) {
                            poojaString.add(
                                "\n*${bp.person[i].id?.trim()})*\nBill Date: *${bp.person[i].date?.trim()}*,\nCategory Name: *${bp.person[i].dietyName?.trim()}*\nProduct: *${bp.person[i].poojaName?.trim()}* - *${bp.person[i].rate}*");
                          }
                          ...
                          // ── GST ───────────────────────────────────────
                          final double cgst =
                              double.tryParse(bp.cgstController.text) ?? 0.0;
                          final double sgst =
                          ...
                          if (gstTotal > 0) {
                            extraLines.write(
                                "\nGST Total: *Rs ${gstTotal.toStringAsFixed(2)}*");
                          }

                          String text =
                              'Date: *${bp.summary?.billDate?.trim()}*\n\n'
                              'Bill No: *${bp.summary?.id}*\n\n'
                              'Name: *${bp.temple?.name?.trim()}*\n\n'
                              'Email: *${bp.temple?.email?.trim()}*\n\n'
                              'Branch: *${bp.summary?.counter?.trim()}*\n\n'
                              'Product Details:\n${poojaString.toString().substring(1, poojaString.toString().length - 1)}'
                              '${extraLines.toString()}\n\n'
                              'Payment Mode: *${bp.summary?.mode}*\n\n'
                              'Total Amount: *${bp.summary?.total?.trim()}*\n\n'
                              'Amount Paid: *${bp.summary?.recvAmt?.trim()}*\n\n'
                              'Book Online: *${bp.temple?.website}*';

                          bp.launchWhatsAppUri(text);
                        }
                        widget.trans.clear();
                        widget.previewBillProvider.clearValues();
                        widget.previewBillProvider.previewDetailsList.clear();
