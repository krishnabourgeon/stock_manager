...
class _SupplierScreenState extends State<SupplierScreen>
  ...
  // Edit mode — holds the Data object being edited
  Data? _editingSupplier;
  ...
  Future<void> _submitForm() async {
    ...
    if (_editingSupplier != null) {
      // UPDATE — local only (wire up an update API if available)
      _showSnackBar("Supplier updated successfully", Colors.green);
      _clearForm();
    ...
    // ── ADD via API ────────────────────────────────────────────────────────
    final contactNoRaw = _contactNumberController.text.trim();
    final contactNo = int.tryParse(contactNoRaw);
    if (contactNo == null) {
      _showSnackBar("Enter a valid numeric phone number", Colors.red);
      return;
    }

    setState(() => _isSubmitting = true);

    await context.read<StockProvider>().saveSupplier(
      name: _supplierNameController.text.trim(),
      contactPerson: _contactPersonController.text.trim(),
      contactNo: contactNo,
      address: _addressController.text.trim(),
      onSuccess: () {
        // Refresh the supplier list so View tab is up-to-date
        context.read<StockProvider>().getSuppliers();
  ...
  void _populateForEdit(Data supplier) {
    setState(() => _editingSupplier = supplier);
    _supplierNameController.text = supplier.name ?? '';
    _contactPersonController.text = supplier.contactperson ?? '';
    _contactNumberController.text = supplier.contactno?.toString() ?? '';
    _addressController.text = supplier.address ?? '';
  ...
  Widget _buildViewTab() {
    return Consumer<StockProvider>(
      builder: (context, provider, _) {
        ...
        final isLoading = provider.loaderState == LoaderState.loading;
  ...
  // SUPPLIER CARD
  // ---------------------------------------------------------------------------
  Widget _buildSupplierCard(Data supplier, int index) {
    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow(
                Icons.person_outline, "Contact Person", supplier.contactperson ?? '—'),
            const SizedBox(height: 8),
            _buildDetailRow(
                Icons.phone_outlined, "Phone", supplier.contactno?.toString() ?? '—'),
            const SizedBox(height: 8),
            _buildDetailRow(
