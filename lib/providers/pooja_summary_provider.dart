...
class PoojaSummaryProvider extends ChangeNotifier with ProviderHelperClass {
  ...
  Future<void> getPoojaSummary({
    bool enableLoaderState = true,
    int? categoryId,
    int? productId,
  }) async {
    // Cache filters so date-change re-fetches also include them
    if (categoryId != null) activeCategoryId = categoryId;
    if (productId != null) activeProductId = productId;

    if (enableLoaderState) updateLoadState(LoaderState.loading);
    if (!enableLoaderState) updatePaginationLoader(true);

    try {
      var res = await serviceConfig.getPoojaSummary(
        fromDate ?? '',
        toDate ?? '',
  ...
  /// Call this when the user clears all filters
  void clearCategoryProductFilter() {
  ...
  updatePoojaDataList(PoojaSummaryResponse? poojaSummaryResponse) {
    if (pageCount == 1) {
      ...
      tempPoojaDataList = poojaSummaryResponse?.data ?? [];
      List<PoojaSummaryData> tempList = [...poojaDataList];
      poojaDataList = [...tempList, ...poojaSummaryResponse?.data ?? []];
    }
